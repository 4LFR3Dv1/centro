...
const STATIONS = [
  ...
  { name: 'Estação Vila Sanches', aliases: ['vila sanches'], queries: ['Avenida Doutor Nelson D Avila, 2740, Jardim São Dimas', 'Avenida Doutor Nelson D Avila, Vila Sanches'] },
  ...
  { name: 'Estação Vila Bandeirantes', aliases: ['vila bandeirantes', 'frederico ozanam', 'rodoviaria'], queries: ['Rua Antônio Porfírio da Silva, 150, Jardim Paulista', 'Terminal Rodoviário Frederico Ozanam'] },
...
async function fetchJson(url, options = {}) {
  const response = await fetch(url, {
    ...options,
    headers: { 'user-agent': 'Centro-Transito/0.6', ...(options.headers ?? {}) },
...
function mappedStation(stopFeatures, station) {
...
async function resolveStations(stopFeatures) {
  ...
  for (let index = 0; index < STATIONS.length; index += 1) {
    ...
    if (!coordinates) {
      const mapped = mappedStation(stopFeatures, station);
      if (mapped) {
        coordinates = mapped.geometry.coordinates;
        coordinateSource = 'openstreetmap-transit-point';
      }
    }

    if (!coordinates) {
      coordinates = await boundedGeocode([station.name, ...station.queries]);
      if (coordinates) coordinateSource = 'bounded-geocode';
    }
    ...
    features.push({
      type: 'Feature',
      properties: {
        kind: 'linha-verde-station',
        name: station.name,
        sequence: index + 1,
        officialNameSource: OFFICIAL_LINE_GREEN,
        coordinateSource,
      },
...
async function officialRoutes() {
  const response = await fetch(OFFICIAL_TRANSPORT, {
    headers: { 'user-agent': 'Centro-Transito/0.6' },
...
function assertSnapshot(geojson, manifest, summary, raw) {
  ...
  return { stations: stations.length, transitPoints: transitPoints.length, routes: summary.officialRouteDirectoryCount };
}

async function validatePersisted() {
  const [rawGeo, rawManifest, rawSummary] = await Promise.all([
  ...
  console.log(`Validated transit snapshot: ${result.stations} Linha Verde stations · ${result.transitPoints} mapped stops/terminals · ${result.routes} official directory entries`);
}
...
async function main() {
  ...
  const summary = {
    ...
    linhaVerde: { stationCount: 13, geometryStatus: 'stations-only' },
    mappedStopsAndTerminals: stopFeatures.length,
    officialRouteDirectoryCount: routes.length,
    officialRouteDirectory: routes,
    note: 'Os nomes e a ordem das estações da Linha Verde vêm da Prefeitura. As coordenadas servem à visualização e são resolvidas dentro de São José dos Campos. O Centro não desenha um traçado viário exato da Linha Verde sem geometria validada.',
  };
  const manifest = {
    schemaVersion: 1,
    area: 'São José dos Campos, SP, Brasil',
    source: {
      official: 'Prefeitura de São José dos Campos',
      mappedStops: 'OpenStreetMap via Overpass',
      stationCoordinates: 'OSM transit points / bounded geocoding / persisted coordinates',
      licenseOSM: 'ODbL 1.0',
    },
