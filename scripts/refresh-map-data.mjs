...
const NEGATIVE_CYCLEWAY_VALUES = new Set(['no', 'none', 'separate']);
...
function positiveCyclewayValue(value) {
  const normalized = compact(value).toLowerCase();
  if (!normalized) return false;
  return normalized
    .split(/[;,]/)
    .map((part) => part.trim())
    .some((part) => part && !NEGATIVE_CYCLEWAY_VALUES.has(part));
}

function hasCycleFacility(tags) {
  if (compact(tags.highway).toLowerCase() === 'cycleway') return true;
  return [
    tags.cycleway,
    tags['cycleway:left'],
    tags['cycleway:right'],
    tags['cycleway:both'],
  ].some(positiveCyclewayValue);
}

function hasCycleFacilityProperties(properties = {}) {
  if (compact(properties.highway).toLowerCase() === 'cycleway') return true;
  return [
    properties.cycleway,
    properties.cyclewayLeft,
    properties.cyclewayRight,
    properties.cyclewayBoth,
  ].some(positiveCyclewayValue);
}
...
function normalize(payload) {
  ...
  for (const element of payload.elements ?? []) {
    ...
    if (!hasCycleFacility(tags)) continue;
    seen.add(element.id);
    ...
    features.push({
      ...
      properties: {
        ...
        cyclewayBoth: compact(tags['cycleway:both']),
...
function assertSnapshot(geojson, manifest, rawGeoJson) {
  ...
  const invalid = geojson.features.find((feature) => !hasCycleFacilityProperties(feature.properties));
  if (invalid) {
    throw new Error(`Snapshot includes a way without positive cycling infrastructure: ${invalid.id ?? 'unknown'}`);
  }
...
async function main() {
  ...
  if (geojson.features.length === 0) throw new Error('No positive cycleway geometry returned for São José dos Campos');
  ...
  const manifest = {
    ...
    selection: 'highway=cycleway or positive cycleway/cycleway:left/cycleway:right/cycleway:both; excludes no, none and separate',
    retrievedAt,
